...
helm upgrade flux fluxcd/flux --install --namespace admin -f ${VALUES} --version 1.3.0 --set image.tag=1.19.0 \
    --set "git.path=k8s/${ENV}/common\,k8s/${ENV}/${CLUSTER_NAME}\,k8s/${ENV}/${CLUSTER_NAME}-overlay\,k8s/common\k8s/${ENV}/common-overlay\",git.label=${ENV},git.email=flux-${ENV}@hmcts.net,git.user="Flux ${ENV}" \
    --wait
