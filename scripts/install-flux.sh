...
FLUX_HELM_CRD=https://raw.githubusercontent.com/fluxcd/helm-operator/chart-0.4.0/deploy/flux-helm-release-crd.yaml
...
helm upgrade flux fluxcd/flux --install --recreate-pods --namespace admin -f ${VALUES} --version 1.0.0 --set image.tag=1.17.0 \
...
helm upgrade flux-helm-operator fluxcd/helm-operator --install --namespace admin   -f  deployments/fluxcd/helm-operator-values.yaml --version 0.5.0 --wait "${helm_tls_params[@]}"
